...
- Бэкенд слушает localhost с открытым CORS: пока он запущен, **любой сайт
  в браузере** может слать запросы к API (и тратить ваши LLM-кредиты).
  Не держите сервер запущенным без необходимости.
- Содержимое просматриваемой страницы попадает в промпт и (при включённом
  Langfuse) в трейсы Langfuse Cloud.
- Текст страницы может содержать вредные инструкции для агента
  (prompt injection): инструмент `fetch_url` ограничен публичными
  http/https-адресами, но эксфильтрацию данных через параметры URL
  архитектурно не исключить.
